...
public class SkinManager {
    ...
    public void changeSkin(final String skinPluginPath, ISkinChangingCallback callback) {
        changeSkin(skinPluginPath, null, "", callback);
    }
     ...
     * @param pkgName
     * @param suffix 根据suffix选择插件内某套皮肤，默认为""
     * @param callback
     */
    public void changeSkin(final String skinPluginPath, String pkgName, final String suffix,
        ...
        if (TextUtils.isEmpty(skinPluginPath) || !new File(skinPluginPath).exists()) {
            callback.onError(new NullPointerException("skin file not exists!"));
            return;
        }
        if (TextUtils.isEmpty(pkgName)) {
            PackageManager mPm = mContext.getPackageManager();
            PackageInfo mInfo = mPm.getPackageArchiveInfo(skinPluginPath, PackageManager.GET_ACTIVITIES);
            if (mInfo == null) {
                callback.onError(new NullPointerException("skin file is not a apk file!"));
                return;
            }
            pkgName = mInfo.packageName;
        }
        ...
        final String skinPkgName = pkgName;
        new AsyncTask<Void, Void, Void>() {
            @Override
            protected Void doInBackground(Void... params) {
                try {
                    loadPlugin(skinPluginPath, skinPkgName, suffix);
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            protected void onPostExecute(Void aVoid) {
                try {
                    updatePluginInfo(skinPluginPath, skinPkgName, suffix);
                    notifyChangedListeners();
